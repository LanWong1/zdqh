...
//设置 每一列的实际宽度，确定表的总宽度
-(void)setColumnWidths:(int)allColumns withMaxWidth:(int)maxWidth{
    _allColumns = allColumns;
    _columnWidths = [[NSMutableArray alloc]initWithCapacity:_allColumns];//多少列
    float count = 0;
    for(int i = 0;i < allColumns; i++){
     //w = size.width * ([columnWidths[i] floatValue]) + 6;
       float w = maxWidth/allColumns+20;
       count += w;
...
//                }
    ...
    for (int section = 0; section < [self.collectionView numberOfSections]; section ++) {
        ...
        for (NSUInteger row = 0; row < _allColumns; row++) {
            ...
            if (section == 0 && row < _lockColumn) {
                attributes.zIndex = 14; // 设置这个值是为了 让第一项(Sec0Row0)显示在第一列和第一行
            } else if (section == 0 || row < _lockColumn) {
                attributes.zIndex = 13; // 设置这个是为了 让第一行在上下滚动的时候固定和锁定的列在左右滚动的时候固定，这个值要比上面的小要比0大
            }
            //首行
            if (section == 0) {
                CGRect frame = attributes.frame;
